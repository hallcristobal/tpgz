...
namespace InputViewer {
    ...
    void draw_cross(uint32_t color, Vec2 pos, float size) {
        ...
        draw_button(Controller::DPAD_LEFT, color, {pos.x, pos.y + branch_length}, {branch_length, branch_width});
        draw_button(Controller::DPAD_UP, color, {pos.x + branch_length, pos.y}, {branch_width, branch_length});
        draw_button(Controller::DPAD_RIGHT, color, {pos.x + branch_length + branch_width, pos.y + branch_length}, {branch_length, branch_width});
    ...
    void draw_input_viewer(Font& font, Vec2 pos, float scale, bool is_shadow) {
        ...
        draw_button(Controller::Y, is_shadow ? 0x00000060 : 0xFFFFFFFF, {pos.x + 130.f * scale, pos.y + 7.5f * scale}, {30.f * scale, 15.f * scale});
        draw_button(Controller::Z, is_shadow ? 0x00000060 : 0x8A2BE2FF, {pos.x + 167.5f * scale, pos.y + 7.5f * scale}, {15.f * scale, 15.f * scale});
        draw_button(Controller::START, is_shadow ? 0x00000060 : 0xFFFFFFFF, {pos.x + 87.5f * scale, pos.y + 40.f * scale}, {10.f * scale, 10.f * scale});
        draw_cross(is_shadow ? 0x00000060 : 0xFFFFFFFF, {pos.x + 95.f * scale, pos.y + 10.f * scale}, 25.f * scale);
